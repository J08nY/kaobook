...
in a UNIX system, but see also \ncref{compiling}; I have no idea about 
...
hyperreferences for this, with the \Command{label}-\Command{ref}
mechanism. There are three useful packages that can make the references
look more like what you want: \Package{cleveref}, \Package{nameref},
and \Package{varioref}.\sidenote{\Package{zref} is a newer and
promising alternative, but it will not be covered here.} Of these,
\Package{cleveref} is the most powerful, as it can automatically
infer the type of element that you are referring to (figure, table,
section, equation, \ldots), and add an appropriate prefix. For
example, \Command{ref{my-figure}} becomes \enquote{Figure 1} and you
don't have to type \enquote{Figure} yourself; \Package{cleveref}
also takes care of capitalization and lets you change the prefixes
globally. \Package{nameref} allows you to print the name of the section
you are referring to, instead of just its number. For instance,
\Command{nameref{my-section}} becomes \enquote{Title of the section}.
Finally, \Package{varioref} adds some spice to your references by adding
the page number and, for instance, replacing \enquote{on page 99} with
\enquote{on the following page} whenever appropriate.

In previous \Class{kaobook} versions, I introduced bespoke labelling and
referencing commands, specific to each element. Thus, there would be
\Command{labch} for labelling chapters, \Command{labfig} for figures,
...
with just four commands, using \Package{cleveref} features.

Labelling in \Class{kaobook} is no different from labelling in regular
\LaTeX: simply use \Command{label{my-label}} right after the element
...
Standard \LaTeX\ defines the \Command{ref} command, which you can
of course also use in \Class{kaobook}. However, I recommend to use
\Command{cref} instead, as a drop-in replacement. In addition, since the
style of the reference should depend on the context, \Class{kaobookc}
...
\lstinline|\cref{my-label}| (directly from the \Package{cleveref}
package). Such a command results in: \cref{references}.

The vario and name styles rest upon the \Package{varioref}
package. Their syntax is \lstinline|\vcref{my-label}| and
\lstinline|\ncref{my-label}|, and they result in: \vcref{references},
for the vario style, and: \ncref{references}, for the name style. As you
can see, the page is referenced in \Package{varioref} style.
...
In order to have access to these commands, the package \Package{kaorefs}
...
\Package{plaintheorems} (or \Package{mdftheorems}). \Package{cleveref}
is language-aware and will change the captions based on your document's
language, as long as you use \Package{babel} or \Package{polyglossia}.
It can also be customized to achieve exactly the reference style that
you want. Please refer to the \Package{cleveref} documentation to learn
more.
